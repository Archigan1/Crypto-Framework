var __create = Object.create;
var __defProp = Object.defineProperty;
var __getOwnPropDesc = Object.getOwnPropertyDescriptor;
var __getOwnPropNames = Object.getOwnPropertyNames;
var __getProtoOf = Object.getPrototypeOf;
var __hasOwnProp = Object.prototype.hasOwnProperty;
var __markAsModule = (target) => __defProp(target, "__esModule", { value: true });
var __export = (target, all) => {
  __markAsModule(target);
  for (var name in all)
    __defProp(target, name, { get: all[name], enumerable: true });
};
var __reExport = (target, module2, desc) => {
  if (module2 && typeof module2 === "object" || typeof module2 === "function") {
    for (let key of __getOwnPropNames(module2))
      if (!__hasOwnProp.call(target, key) && key !== "default")
        __defProp(target, key, { get: () => module2[key], enumerable: !(desc = __getOwnPropDesc(module2, key)) || desc.enumerable });
  }
  return target;
};
var __toModule = (module2) => {
  return __reExport(__markAsModule(__defProp(module2 != null ? __create(__getProtoOf(module2)) : {}, "default", module2 && module2.__esModule && "default" in module2 ? { get: () => module2.default, enumerable: true } : { value: module2, enumerable: true })), module2);
};
__export(exports, {
  calculateHash: () => calculateHash,
  getHash: () => getHash,
  makeRoot: () => makeRoot
});
var import_crypto = __toModule(require("crypto"));
var import_MerkleNode = __toModule(require("./MerkleNode"));
const getHash = (data) => {
  return (0, import_crypto.createHash)("sha256").update(data.toString()).digest("hex");
};
const makeRoot = (arr) => {
  if (arr.length === 1)
    return arr[0];
  const list = [];
  const length = arr.length;
  for (let i = 0; i < length; i += 2) {
    const currentItem = arr[i];
    if (i + 1 >= length) {
      list.push(currentItem);
      break;
    }
    const nextItem = arr[i + 1];
    let value = currentItem.value + nextItem.value;
    const node = new import_MerkleNode.default(getHash(value), currentItem, nextItem);
    list.push(node);
  }
  return makeRoot(list);
};
const calculateHash = (block) => {
  const blockData = block.rootHash + block.previousHash + block.timestamp.toISOString() + block.pow.toString();
  return (0, import_crypto.createHash)("sha256").update(blockData).digest("hex");
};
// Annotate the CommonJS export names for ESM import in node:
0 && (module.exports = {
  calculateHash,
  getHash,
  makeRoot
});
//# sourceMappingURL=utils.js.map
